...
{
    char    nome_fich[100];
    int     vert,  num_iter = 10000, k, runs, custo, best_custo, kFile;
    int     *grafo, *sol, *best;
    float   mbf = 0.0;
    ...
    if (runs <= 0)
        return 0;
    init_rand();
    grafo = init_dados(nome_fich, &vert, &kFile);
    //printf("!%d!", kFile);
    ...
    for(k=0; k<runs; k++)
    {
        ...
        gera_sol_inicial(sol, vert, kFile);
        ...
        custo = trepa_colinas(sol, grafo, vert, num_iter, kFile);
